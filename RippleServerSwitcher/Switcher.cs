...
namespace RippleServerSwitcher
{
    ...
    class Switcher
    {
        ...
        {
            new HostsEntry{domain="osu.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="c.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="ce.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="a.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="s.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="i.ppy.sh", ip="163.172.67.35"},
            new HostsEntry{domain="bm6.ppy.sh", ip="51.15.222.176"}
        }.Concat(
            (from x in Enumerable.Range(1, 6) select
                new HostsEntry{ domain=String.Format("c{0}.ppy.sh", x), ip="163.172.67.35" }
            ).ToArray()
        ).ToArray();
    ...
    private HostsFile hostsFile = new HostsFile();
        ...
        public CertificateManager CertificateManager = new CertificateManager { SubjectName = "Hangout CA", Certificate = Resources.Certificate };
        ...
        public async Task ConnectToRipple()
        {
            ...
            await hostsFile.Parse();
            if (!IsConnectedToRipple())
                throw new HumanReadableException("Can't write entries. Disable antivirus.", "The hosts file looks writable, but Akatsuki Server Switcher wasn't able to write the required entries to it and switch to Akatsuki. There is most likely a software blocking access to the hosts file. Please disable your antivirus, third party firewall or any similar software that may block edits on the hosts file.");
        }
        ...
        public async Task UpdateIPs()
        {
            ...
            try
            {
                using (var result = await httpClient.GetAsync("https://old.akatsuki.pw/current.json"))
                {
